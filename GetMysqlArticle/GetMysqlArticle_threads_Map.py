...
def Download(url_name,location):
	for key in url_name:
		local = os.path.join(location,str(url_name[key]))
		try:
			print "%s "%url_name[key]
			print key 
			print "Downloading..."
			urllib.urlretrieve(key,local)
		except socket.timeout:
			print 'timeout: \n',key
		except Exception,e:
			print 'error:%s \n'%e
...
'''
	locals = os.path.join(location,filename)
...
def DownloadTOLoal():
	...
	# split rest_a for thread which using MapReduce
	global Threads_number
	union_item=[]
	i = 0
	for item in chunks(rest_a,len(rest_a)//Threads_number):
		i = i+1	
		union_item.append(item)
	
	#pdb.set_trace()

	# create threading object
	threads = [Thread(target=Download,args=(union_item[ii],location)) for ii in xrange(i) ]
        	
	# start and wait every threading end
	for t in threads:
		t.start()
	for t in threads:
		t.join()
